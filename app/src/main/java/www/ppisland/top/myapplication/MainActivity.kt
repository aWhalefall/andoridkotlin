...
import www.ppisland.top.myapplication.fragmentview.gif.GifFragment
import www.ppisland.top.myapplication.fragmentview.text.TextFragment
import java.io.File
...
fun showSnackbar(viewGroup: ViewGroup, text: String, duration: Int = 1000) {
    ...
    snack.setAction("Action",View.OnClickListener {
        Toast.makeText(App.instance,"展示",Toast.LENGTH_SHORT).show()
    })
    snack.show()
}
...
//    var spunits= preference(this,"sp_key",0)
//
//    var spUtils= this.getSharedPreferences("default", Context.MODE_PRIVATE);


    //委派方式
    // var mDefaultIndex: Int by preference(this@MainActivity, "sp_key_default_fragment", 0)

    var mDefaultIndex: Int = 0
    ...
    override fun onKeyDown(keyCode: Int, event: KeyEvent?): Boolean {
        when (keyCode) {
            KeyEvent.KEYCODE_BACK -> mBackPressedTime = if (mIsMenuOpen) {
                drawerLayout.closeDrawers()
                mBackPressedTime
            } else {
